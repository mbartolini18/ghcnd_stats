...
title=stname+' | Annual Snowfall Time Series | Period of Record: '+syr+'-'+eyr #Plot title
...
for i,mask in enumerate(water_yr_masks[1:]): #Skip first (partial) year
            ...
            if len(dfwatyr.SNOW_ACC.values)>364 & len(dfwatyr.SNOW_ACC.values)<367:
...
top5=list(snow_acc_years_sorted.index[:5])
bot5=list(snow_acc_years_sorted.index[-5:])
...
ax.plot(dates,gaussian_filter(q50,sigma=5),color='0.5',lw=2,label='50th Percentile ('+str(np.around(q25[-1],decimals=1))+' in.)',zorder=2.7)
